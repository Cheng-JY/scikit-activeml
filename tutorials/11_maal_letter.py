...
def load_dataset_letter(random_state=42):
    ...
    X = np.load(f'{data_dir}/letter-X.npy').astype(np.float32)
...
if __name__ == '__main__':
    ...
    # load dataset
    X_train, X_test, y_train, y_test, y_train_true, y_test_true = load_dataset_letter(RANDOM_STATE)
    ...
    for c in range(n_al_cycle+1):
        ...
        misclassification.append(1-accuracy)
        print(c, accuracy)
